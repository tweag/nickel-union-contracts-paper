...
const constantContract = Type.intersection(Type.fun([Base.number], Type.fun([Base.number], Base.number)),
    Type.fun([Base.number], Type.fun([Base.boolean], Base.number)));

const weirdContract = Type.union(Base.any, Type.fun([Type.fun([Base.number], Base.number)],
    Type.fun([Base.number], Base.number)));

const hoIdN = Type.fun([Type.fun([Base.number], Base.number)], Type.fun([Base.number], Base.number));
const hoIdB = Type.fun([Type.fun([Base.number], Base.boolean)], Type.fun([Base.number], Base.boolean));

const hoId = Type.intersection(hoIdB, hoIdN);
...
describe("Intersection", () => {
    ...
    describe("with second level higher order", () => {
        function cons(x: any) {
        ...
        const consWrap = contract.assert(cons, constantContract);
        const cons3 = consWrap(3);

        it("should not throw (at same place)", () => {
            expect(() => (cons3(45) + cons3(true))).to.not.throw();
        });

        const cons3bi = consWrap(45);

        it("should not throw (first app)", () => {
            expect(() => cons3bi(true)).to.not.throw();
        });

        it("should not throw (second app)", () => {
            expect(() => cons3bi(454)).to.not.throw();
        });



        it("should throw Negative", () => {
            expect(() => consWrap("hello")).to.throw(NegativeBlame);
        });

        it("should throw Negative", () => {
            expect(() => consWrap(2)("hello")).to.throw(NegativeBlame);
        });

    });

    describe("positive and then negative", () => {
        function cons(x: any) {
            return function (y: any) {
                return x;
            }
        ...
        function f(g: any) {
            return cons(id)(g(true));
        }


        const fWrap = contract.assert(f, weirdContract);

        it("should throw", () => {
            expect(() => (fWrap(id)(false))).to.throw(NegativeBlame);
        });
    ...
    describe("deep shit", () => {
        ...
        const wrapid = contract.assert(id, hoId);

        const wrapF = wrapid(f);

        it("should throw", () => {
            expect(() => wrapF(4)).to.not.throw();
        });
