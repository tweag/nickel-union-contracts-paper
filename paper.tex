...
intersections types are difficult to verify statically, it may appear
at first that the dynamic part of gradual typing is actually pretty
simple.

It turns out however, that in presence of higher-order contracts union
and intersection is deceptively difficult. The literature on
higher-order contracts with union and intersection, while keenly aware
of the fact, doesn't really explain why. We point and illustrate the
problems and trade-offs inherent to union and intersection contracts,
via example and a survey of the literature.
\end{abstract}
...
\typescript{Array< 42 | "Nickel" >} or an \typescript{Array< "Nickel" | 3.5 >},
where \typescript{42}, \typescript{3.5}, and \typescript{"Nickel"} are literal types
\footnote{Note that in TypeScript, union and intersection are written as
\typescript{|} and \typescript{&}, instead of \nickel{@|} and \nickel{@&},
as we do in Nickel.}.
What value could we fill the square ($\square$) with for this to typecheck?
Since we don't know what's the shape of \typescript{list},
we can't know whether to push a \typescript{42 | "Nickel"} or a
\typescript{"Nickel" | 3.5}. In general,
when using a union, we have to treat it as if it where any one
of the alternatives. In the case of the example, we should provide
it with a value that could be pushed to both arrays, the only possible one
is \typescript{"Nickel"},
since it's the only value that satisfies the intersection of
\typescript{42 | "Nickel"} and \typescript{"Nickel" | 3.5}.

\info{Not planned, but nice place to smooth things out towards
intersections}

\begin{figure}[h]
\begin{lstlisting}[language=JavaScript]
function addElem(list: Array< 42 | "Nickel" > | Array< "Nickel" | 3.5 >) {
    list.push((*@ $\square$ @*))
}
\end{lstlisting}
...
% failed, and marking \nickel{Str -> Str} as failed negatively on \nickel{l}
% is not enough, the contract \nickel{Str -> Str} should not have been used
...
we could choose a semantics for intersections that validate this rule.  However,
...
\resolved{(Yann) I just reworked the wording, but I'm not totally sold on the
argument
(Teo) I agree, I think it would need to be expanded for it to actually
say something, I'll remove it}
...
This transformation flattens arbitrarily nested unions and
...
\resolved{Arnaud says: this whole
...
On the semantic side, one of the so-called sound monitoring properties
...
(as in Figure~\ref{fig:naive-impl} for intersections): \racket{and/c} (resp.  \racket{or/c})
