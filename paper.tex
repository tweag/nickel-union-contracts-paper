...
Unfortunately, this doesn't scale to higher-order contracts.
Figure~\ref{fig:inter-contracts} illustrates the use of an intersection to model
a simple overloading of the identity function. If we were to apply the naive
equivalence above, the argument \nickel{1} would fail the \nickel{Str -> Str}
contract. The naive decomposition actually implies

\begin{lstlisting}[language=nickel,frame=none,numbers=none]
(A -> B) @& (C -> D) (*@ $\simeq$ @*) (A @& C) -> (B @& D)
\end{lstlisting}

This isn't the correct semantics for overloading. With this implementation, our
contract for identity from Figure~\ref{fig:inter-contracts} would always
fail because no argument satisfies \nickel{Num @& Str}.
