...
even number, when applied to a $Number$:
...
written with flat contracts\cite{DLangContracts}.
...
work.} that allows to define flat contracts, and
...
Even if flat contracts may seem like too much, the ability to make
...
a contract that can be composed with other type constructors\unsure{Needs code
...
a value is indeed a member of the corresponding type. Contracts for various
