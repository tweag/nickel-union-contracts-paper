...
to need a matrix of job, wherein the same tests are run on different
infrastructures, or with different versions of a compiler. Traditional
configuration would have you write the same steps for each
infrastructure. It's much better to write the steps once, and
...
configuration have all the required fields? does this field indeed
contains a URL?

These are an inherently dynamic question, as they are all questions
...
..., fieldn = valuen\}}. The application of a contract \nickel{C} to the
expression \nickel{exp} is written \nickel{exp | C}\footnote{The current
    implementation requires a \nickel{#} character in front of custom contracts,
    whose application would rather be \nickel{exp | #C}, but it is irrelevant
and subject to change in the future.}. We will sometimes attach
contracts directly to an identifier instead of the corresponding bound
expression, that is: \nickel{let id | C = value in exp} is syntactic sugar for
\nickel{let id = (value | C) in exp}.

Some examples resort to constructions that are not currently available in Nickel
for illustration purpose: we will use \nickel{A @& B} (resp. \nickel{A @| B})
for the intersection (resp. union) of contracts (or types) \nickel{A} and
\nickel{B}. \nickel{@|} and \nickel{@&} bind more loosely than arrows, meaning
that \nickel{A -> B @& C -> D} is to be parsed as \nickel{(A -> B) @& (C -> D)}.
...
% LocalWords:  Felleisen JSON TOML YAML templating Jsonnet Dhall
% LocalWords:  Kubernetes DevOps natively
