...
given in Figure~\ref{fig:naiv-impl}, which is a direct consequence of the naive
...
overloaded identity of Figure~\ref{fig:naiv-impl} would always fail because
...
as clearly wrong with respect to overloading. There is nothing wrong about it
per se : we could chose a semantics for intersections that validate this rule.
However, the resulting system wouldn't capture overloading, and is arguably not
very interesting nor useful. In~\ref{KeilThiemannUnionIntersection}, the authors
argue for basing the semantics of union and intersection contracts on union and
intersection type systems, where they have a simple and natural definition from
which the other properties follow. This approach makes sense, and has been
continued in future works, and in particular in~\ref{RootCauseOfBlame}.

But even this semantics is no panacea. Figure~\ref{fig:intersection-distrbution}
shows an example that intuitively looks like it should succeeed, returning
\nickel{(1, 1)}. However, under the semantics used both
in~\ref{KeilThiemannUnionIntersection} and~\ref{RootCauseOfBlame}, this program
...
This discrepancy is inherent to the chosen semantics for intersections.  When
applying a function with an intersection contract, only one fixed component of
the intersection can be selected for the function.
one components of the intersection. Not that it must necessarily known right at the
application, but the result can't be used
functions, the choice of the component is fixed once for all when the function
is applied. In Figure~\ref{fig:intersection-distrbution}, the contract for
\nickel{h} can be either \nickel{Num -> Num} or \nickel{Bool -> Num}, but can't
be both. This is a manifestation of the following more general fact: the
distributivity law given in Figure~\ref{fig:distributivity-law} is not valid in
the semantics, while it is both intuitive and sound.\unsure{What means sound, in
this context?}
