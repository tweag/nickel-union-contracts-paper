...
contracts (or types) \nickel{A} and \nickel{B}. Finally, we will write
\nickel{exp @ C}\footnote{The current Nickel implementation actually uses the
syntax \nickel{exp | C} for contract application, but we rather keep \nickel{|}
for unions here}  for the application of a contract \nickel{C} to the expression
\nickel{exp}.
