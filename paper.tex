...
\unsure{Make sure that this point is made in the user-defined
  contracts section: With respect to
...
shape is known to be in a fixed set of alterniteves, without exposing
...
They can also be used to combine records \unsure{Better
naming}. For instance, in Figure \ref{fig:intersection-record}
...
the other hand, just need to verify a property for a specific and known value,
...
function contracts can be implemented in a few line of code (they are
implemented in e.g. Racket~\cite{}).
...
attached of Figure~\ref{fig:pos-to-pos}. The program fails with a contract error
blaming \nickel{f}. Indeed, the second call \nickel{f 5} fails because the
return value violates the \nickel{Positive} contract. The first call to
\nickel{f} does not matter, and \nickel{f 5} is a single and independent witness
of the violation of the contract. The user can be pointed to one location in
practice.
...
function contract to \nickel{f} replaces it with a \nickel{f'} that additionally
performs the required checks. Thus, since the core language is pure (the
presence of \nickel{fail} isn't relevant here), the failure of \nickel{f' 5}
...
Figure~\ref{fig:pos-to-pos}, removing only one the call makes the program
succeed! Indeed, the first call only fails the \nickel{Positive -> NonPositive}
part of the union, while the second one only fails the \nickel{Positive ->
NonPositive} part. In this particular example, there can't be a single call that
is a witness of the violation of the contract: a minimum of two are needed.

This behavior indicates that union contracts introduce side-effects. The result
of \nickel{f 5} now depends on the previous execution and more generally on any
prior call to \nickel{f}. This behavior of union contracts break the
equation~\ref{eq:cbn-expansion} of Section~\ref{sec:optimizations}. The
candidate example of Figure~\ref{fig:optimizations-let-floating-ex} can't be
optimized in general.

Figure~\ref{fig:optimized-programs} illustrates this point further. There is an
original program, and an optimized version the common subexpression \nickel{f 1}
has been eliminated. While equivalent in a pure language without contracts,
these two programs behave differently here:
...
the two calls refers to the same union contracts, and shows that \nickel{f}
...
\begin{itemize}
    \item For the programmer, they are hard to reason about. They prevent local
        ...
        location.
    \item For the interpreter (or compiler), they inhibit many optimizations and
...
\subsection*{Intersection of user-defined contracts}
...
and \nickel{B} sequentially. This intuition works for simple contracts: surely
checking that \nickel{x | Natural @& Odd} amounts to check that \nickel{x |
Natural} and \nickel{x | Odd}. Unfortunately, this doesn't scale to higher-order
contracts.

\unsure{WARNING: what is below is being reworked by Yann}

works for a lot of simple contracts: Stop for
a second and think how you could implement an intersection contract. The first
thought you may have is "just apply both contracts", as shown on the
Implementation snippet from Figure \ref{fig:inter-contracts}.  Of course, if it
were that simple we wouldn't be writing this paper.  This idea starts to show
problems when you first combine it with higher order contracts, since it would
mean not complying with at least one of these contracts negatively would entail
breaking the whole thing, as shown on the second example on the same Figure,
that would fail.
