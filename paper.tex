...
We will use the syntax of Nickel to write most examples throughout this paper.
Syntactically, Nickel looks like a simple ML language with records. \nickel{let
id = value in exp} is used to bind a value to an identifier in an expression.
Functions are declared using the \nickel{fun arg1 ... argn => body} syntax. List
are written as \nickel{[x1, ..., xn]} and records as \nickel{\{ field1 = value1,
..., fieldn = valuen \}}. Some examples uses constructions that are not
currently available in Nickel for illustration purpose: in particular, we will
use \nickel{A & B} (resp.  \nickel{A | B}) for the intersection (resp. union) of
contracts (or types) \nickel{A} and \nickel{B}.
...
\begin{description}
    \item[(correctness)] The program runs without errors. Errors may be type
        ...
        at least early failures and relevant error messages.\todo{make this
...
This paper is structured as follows, Section \ref{sec:intro} introduces
this paper; Section \ref{sec:feat-lang} outlines various features
of programming languages that may interact badly with union and intersection
contracts.
Section \ref{sec:union-inter} gives a primer on Unions and Intersections,
enough to understand this work.

Section \ref{sec:issues-sem} shows what difficulties or issues, proper of
unions and intersections, one may
encounter when trying to develop a union and intersection contracts.
Section \ref{sec:issues-literature} presents issues and shortcomings found
in existing attempts to solve this problem.
Section \ref{sec:related-work} provides some related work, and Section
...
Strikingly, we will see in section~\ref{sec:issues-sem} that the introduction of unions and
...
\todo{Since nickel uses | for contract application, I'd move unions
...
deduce that contract \nickel{String -> String} does not apply in this case,
skipping it.
...
\nickel{(Number -> Number) & (Boolean -> Boolean)} context.
