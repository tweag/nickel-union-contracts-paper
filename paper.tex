...
can be either a boolean or a string. They corresponds logically to conjunctions.
...
Other use-cases for unions include:
\begin{itemize}
    \item To represent nullable values in JSON (\ref{fig:union-nullable-ex}).
...
\end{itemize}
All of which are useful in a configuration language.
...
For instance, if the value \nickel{V} is contracted\todo{(Yann) Contracted means
something else I think} with
...
special cases can hinder composition."\cite{RootCauseOfBlame}\info{(Yann) Does
...
to implement or a difficult to understand system\unsure{(Yann) Is it really a
