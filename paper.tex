...
The good performances of modern programs heavily rely on complex optimizations
performed by the compiler or the interpreter. Even more so for functional
languages, because the functional which model is very different from what actual
computers are good at executing, naive code generation can exhibit unacceptable
slowdowns. Functional programs tend to make heavy use of functions, and function
is a costly operation from the point of view of low-level execution.

One such crucial optimization is inlining. Inlining eliminates a function
application by substituting the function for its definition at compile time (or
before execution, for interpreted language). This is especially important for
short functions that are called repeatedly.
...
Beyond CSE, there exists other similar transformations, such as Haskell's
let-floating~\ref{} or loop-invariant code motion, which amount to hoist
invariant out from repeated blocks.

More generally, there is a whole family of opitmizations, such as let-floating,
\begin{lstlisting}[language=caml]
type Ordering = Smaller | Equals | Greater
sort : 'a list
       -> ('a -> 'a -> Ordering)
       -> 'a list
random_list : unit -> int list

sort (random_list ()) (fun x y ->
  if x < y then Smaller
  else if x == y then Equals
  else Greater)
\end{lstlisting}

Common subexpression elemination (CSE) etc.
