...
data being invalid or inconsistent. Data validation isn’t supported either, and must be handed over to yet another tool down the configuration pipeline, if ever
done. Correctly configuring a modern system is hard and failures may have
substantial negative consequences.

This is why the authors have been developping the Nickel
...
the complexity of a codebase grows, static types becomes attractive again.  For
reusable code — that is, library functions —, static types are specifically
adapated, and bring all the usual benefits of early error detection, code
...
type. Contracts for various extensions of the simply typed $\lambda$-caluclus
...
to encode data schemas encoutered in practice. Since these schemas do use
untagged unions, there is a strong incentive to support untagged uniohns at the
