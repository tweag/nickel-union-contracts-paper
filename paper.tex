...
maps \nickel{0} to \nickel{0}, as can be seen on Figure
\ref{fig:0-to-0}.
...
\end{lstlisting}
    \end{center}
  \caption{A contract that checks that a function maps 0 to 0}
  \label{fig:0-to-0}
\end{figure}
...
consist in introducing a new identifier for this expression and using the
...
can be either a boolean or a string. They correspond logically to conjunctions.
\unsure{disjunctions, right?}
...
Intersections are less prevalent. An intersection is a value that belongs to to
...
\subsubsection*{Union contracts are hard to brake}
...
that checks that maps positive numbers to positive numbers, like
...
the stack trace is enough to understand how it was violated.
...
both function calls are needed, and, what is more, there is not a single way to
use the wrapped value \nickel{f} that would find a violation to its contract;
...
However, this two programs behave differently, the first one will return
...
declarative one
(Teo) I think you're right, I'll try to reword it}.
A second, major one,
...
difficult, or even impossible (as the case with common subexpression
elimination)\unsure{(Yann) I think we need to substantiate this claim. If this
