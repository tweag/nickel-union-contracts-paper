...
\newcommand{\racket}[1]{\lstinline[language=lisp]{#1}}
...
languages, whose operational semantics is usually very different from the actual
primitives of modern processors: naive code generation can exhibit unacceptable
slowdowns. Functional programs tend to make heavy use of functions, and a function
call is a costly operation from the point of view of low-level execution.

One such crucial optimization is inlining. Inlining eliminates a function
...
important for small functions that are called repeatedly.
...
Another optimization is common subexpression elimination (CSE). CSE somehow
goes into the other direction: it factors out equal subexpressions to avoid
recomputing them several times.

\begin{lstlisting}[language=nickel]
let elemAtOrLast = fun index list =>
  if index == lists.length list then
...
particular, take the following example:
...
programs with respect to substitution. Inlining requires that one can replace
the application of a function by its body: this is $\beta$-reduction, and it is
hard to think of a language when it is not a valid transformations. On the other
hand, CSE-like transformations require an expansion property, of the form

$$M[N/x] \simeq let~x~=~M~in~M[x]$$

This does not necessarily holds, and is in particular trivially violated in
presence of side effects:

$$let~f~x~=~print~"hi";~(x+1)~in~(f~1, f~1) \\
\not \simeq let~f~x~=~print~"hi";~(x+1)~in~let~y~=~f~1~in~(y,y)$$

However, this property does hold for \emph{pure} terms, that are terms without
side-effects. Pure languages, or languages with a form of effect tracking such
as Haskell, are able to identify such pure terms. Other languages rely on
sub-approximations (for instance, applying optimizations to arithmetic
expressions).
\newpage
